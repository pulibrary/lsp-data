...
###  - Parenthetical notes in 245a are removed and moved to a 500 field.
...
# Generate a field with the translation note removed, and linkage added.
...
def generate_subfields(parallel_tag, seqno, field_content)
  ...
  subfields = field_content.split('$').filter { |sf| sf.size.positive? }
...
marc_reader.each do |record|
  ...
  record_updates.each do |row|
    ...
    matched_fields = record.fields(tag).find { |f| f.to_s[7..].strip == roman_content }
    indicators = "#{matched_fields.indicator1}#{matched_fields.indicator2}"
    new_roman_field = generate_field([tag, '880'], seqno, indicators, matched_fields.to_s[7..], translation_note)
    ...
    record = MarcCleanup.replace_fields(
      field_array: [{ source_field: matched_fields, replacement_field: new_roman_field,
