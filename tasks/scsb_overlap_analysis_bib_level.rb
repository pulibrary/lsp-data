...
add_matches_from_file(file: "#{output_dir}/nypl_scsb_match_keys.tsv", matches: scsb_matches, inst_symbol: :scsbnypl); nil

pul_recap_unique = scsb_matches.select { |_key, sites| sites[:scsbpul] && sites.size == 1 }.size
cul_recap_unique = scsb_matches.select { |_key, sites| sites[:scsbcul] && sites.size == 1 }.size
nypl_recap_unique = scsb_matches.select { |_key, sites| sites[:scsbnypl] && sites.size == 1 }.size
hl_recap_unique = scsb_matches.select { |_key, sites| sites[:scsbhl] && sites.size == 1 }.size
pul_recap_all = scsb_matches.select { |_key, sites| sites[:scsbpul] }.size
cul_recap_all = scsb_matches.select { |_key, sites| sites[:scsbcul] }.size
nypl_recap_all = scsb_matches.select { |_key, sites| sites[:scsbnypl] }.size
hl_recap_all = scsb_matches.select { |_key, sites| sites[:scsbhl] }.size
...
File.open("#{output_dir}/scsb_shared_overlap_statistics.tsv", 'w') do |output|
  ...
  output.write("PUL\t")
  output.write("#{pul_recap_all}\t")
  output.puts(pul_recap_unique)
  output.write("CUL\t")
  output.write("#{cul_recap_all}\t")
  output.puts(cul_recap_unique)
  output.write("NYPL\t")
  output.write("#{nypl_recap_all}\t")
  output.puts(nypl_recap_unique)
  output.write("HL\t")
  output.write("#{hl_recap_all}\t")
  output.puts(hl_recap_unique)
end; nil

File.open("#{output_dir}/scsb_match_keys_per_institution.tsv", 'w') do |output|
  output.puts("Match Key\tPUL Shared IDs\tCUL Shared IDs\tNYPL Shared IDs\tHL Shared IDs")
  scsb_matches.each do |key, institutions|
    output.write("#{key}\t")
    output.write("#{institutions[:scsbpul]&.join(' | ')}\t")
    output.write("#{institutions[:scsbcul]&.join(' | ')}\t")
    output.write("#{institutions[:scsbnypl]&.join(' | ')}\t")
    output.puts(institutions[:scsbnypl]&.join(' | '))
  end
end; nil
